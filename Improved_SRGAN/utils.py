...
def create_data_lists(train_folders, test_folders, min_size, output_folder):
    ...
    创建训练集和测试集列表文件.
        ...
        参数 output_folder: 最终生成的文件列表，为json格式
    """
    print("Creating a list of files... ")
    train_images = list()
    for d in tqdm(train_folders, desc="Creating list of train set..."):
        for i in os.listdir(d):
            img_path = os.path.join(d, i)
            img = Image.open(img_path, mode='r')
            if img.width >= min_size and img.height >= min_size:
                train_images.append(img_path)
    print("There are %d images in the train set." % len(train_images))
    with open(os.path.join(output_folder, 'train_images.json'), 'w') as j:
        json.dump(train_images, j)

    test_images = list()
    for d in tqdm(test_folders, desc="Creating list of test set..."):
        for i in os.listdir(d):
            img_path = os.path.join(d, i)
            img = Image.open(img_path, mode='r')
            if img.width >= min_size and img.height >= min_size:
                test_images.append(img_path)
    print("There are %d images in the test set." % len(test_images))
...
def get_logger():
    ...
    file_handler = logging.FileHandler('./ISRGAN.log')
...
def adjust_learning_rate(optimizer, shrink_factor):
    ...
    """
    print("Adjustment of learning rates...")
